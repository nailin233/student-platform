...
CREATE TABLE IF NOT EXISTS payments (id BIGINT UNSIGNED AUTO_INCREMENT PRIMARY KEY,student_id BIGINT UNSIGNED NOT NULL,amount DECIMAL(10,2) NOT NULL,payment_method VARCHAR(30),payment_time DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,remark VARCHAR(500),FOREIGN KEY(student_id) REFERENCES students(id) ON DELETE CASCADE) ENGINE=InnoDB;
CREATE TABLE IF NOT EXISTS class_changes (id BIGINT UNSIGNED AUTO_INCREMENT PRIMARY KEY,student_id BIGINT UNSIGNED NOT NULL,old_course_id BIGINT UNSIGNED,new_course_id BIGINT UNSIGNED,change_time DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,reason VARCHAR(500),FOREIGN KEY(student_id) REFERENCES students(id) ON DELETE CASCADE) ENGINE=InnoDB;
INSERT INTO courses(name,description,duration,price,class_time) VALUES ('书法班','硬笔与毛笔基础','12周',1280,'周六 09:00-11:00'),('摄影班','手机摄影入门','8周',980,'周日 14:00-16:00'),('声乐班','成人声乐基础','12周',1680,'周三 19:00-21:00') ON DUPLICATE KEY UPDATE name=VALUES(name);
INSERT INTO teachers(name,phone,specialty,introduction) VALUES ('王老师','13800000001','书法','专注成人书法教学'),('李老师','13800000002','摄影','擅长手机摄影教学'),('陈老师','13800000003','声乐','拥有多年声乐教学经验');
